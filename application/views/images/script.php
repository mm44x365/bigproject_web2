<script>
    $(document).ready(function() {
        ...
        table = $('#table').DataTable({
            ...
            "ajax": {
                "url": "<?= base_url() ?>images/ajax_list/<?= $role_list ?>",
                "type": "POST",
            },
    ...
    function add() {
        save_method = 'add';
        $('#submit')[0].reset(); // reset form on modals
    ...
    function edit(id) {
        ...
        $.ajax({
            url: "<?= base_url() ?>images/ajax_edit/" + id,
            type: "GET",
            dataType: "JSON",
    ...
    $(document).ready(function() {
        ...
        $('#submit').submit(function(e) {
            e.preventDefault();
            $.ajax({
                url: '<?php echo base_url(); ?>images/ajax_add/',
                type: "post",
    ...
    function deleteData(id) {
        if (confirm('Apakah anda yakin akan menghapus data ini?')) {
            ...
            $.ajax({
                url: "<?= base_url() ?>images/ajax_delete/" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data) {
    ...
    function resetPassword(id) {
        if (confirm('Password akan direset menjadi 1234, apakah anda yakin?')) {
            ...
            $.ajax({
                url: "<?= base_url() ?>images/ajax_password/" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data) {
        ...
        if (list_id.length > 0) {
            if (confirm('Apakah anda yakin akan menghapus ' + list_id.length + ' data?')) {
                $.ajax({
                    ...
                    url: "<?= base_url() ?>images/ajax_list_delete",
                    dataType: "JSON",
                    success: function(data) {
                        if (data.status) {
